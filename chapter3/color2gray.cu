...
__global__ void colorToGreyscaleKernel(unsigned char *in, unsigned char *out, unsigned int height, unsigned int width) {
...
void colorToGreyscale(unsigned char *in, unsigned char *out, unsigned int height, unsigned int width) {
    ...
    cudaMemcpy(out, d_out, sizeof(unsigned char) * height * width, cudaMemcpyDeviceToHost);
